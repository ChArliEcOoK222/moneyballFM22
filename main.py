...
def defence_pizza(df, player):
    ...
    # add text
    fig.text(
        0.34, 0.925, "Defending        Passing       Dribbling", size=14,
...
def midfielder_pizza(df, player):
    ...
    fig.text(
        0.515, 0.942,
        "Percentile Rank v Top 5 League Midfielders",
        size=15,
        ha="center", fontproperties=font_bold.prop, color="#FFFFFF"
    )
...
def attack_pizza(df, player):
    slice_colors = ["#1A78CF"] * 5 + ["#FF9300"] * 2 + ["#D70232"]
    text_colors = ["#FFFFFF"] * 7 + ["#F2F2F2"]
    df = df[['Name', 'Club', 'Gls/90', 'Shot/90','ShT/90','xG', 'npxG', 'Drb/90', 'Ps C/90', 'Hdrs W/90']]
    df.fillna(0, inplace=True)
    df.set_index('Name', inplace=True)
    club = df.loc[player]['Club']
    df_ranked = df.rank(numeric_only=True, pct=True)
    ...
    fig.text(
        0.515, 0.942,
        "Percentile Rank v Top 5 League Attackers",
        size=15,
        ha="center", fontproperties=font_bold.prop, color="#FFFFFF"
    )
    ...
    # add text
    fig.text(
        0.34, 0.925, "Shooting        Dribbling       Aerial", size=14,
        fontproperties=font_bold.prop, color="#FFFFFF"
    )
...
def goalkeeper_pizza(df, player):
    slice_colors = ["#1A78CF"] * 2 + ["#FF9300"] * 3 + ["#D70232"]
    text_colors = ["#FFFFFF"] * 5 + ["#F2F2F2"]
    df = df[['Name', 'Club', 'Con/90', 'Clean sheets','Svh','Svt', 'Svp', 'Ps C/90']]
    df.fillna(0, inplace=True)
    df.set_index('Name', inplace=True)
    club = df.loc[player]['Club']
    df_ranked = df.rank(numeric_only=True, pct=True)
    ...
    fig.text(
        0.515, 0.942,
        "Percentile Rank v Top 5 League Goalkeepers",
        size=15,
        ha="center", fontproperties=font_bold.prop, color="#FFFFFF"
    )
    ...
    # add text
    fig.text(
        0.34, 0.925, "Conceding        Saving       Passing", size=14,
        fontproperties=font_bold.prop, color="#FFFFFF"
    )
...
goalkeepers = df.loc[df['Position Category'] == 'Goalkeeper']
